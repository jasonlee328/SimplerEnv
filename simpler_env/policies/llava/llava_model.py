...
class LLaVAInference:
    ...
    def unnormalize_action_tokenized(self, generated_text, chosen_probs, amplification_factor=1.0):
        ...
        action_confidence = self.compute_min_confidence_for_token_list(individual_chars, chosen_probs)
        composite_confidence = self.compute_composite_confidence_for_token_list(individual_chars, chosen_probs)
    
        print("Composite Confidence:", composite_confidence)
        print("Action Confidence:", action_confidence)
        logger.info(f"Action Confidence: {action_confidence}")

        self.adjust_threshold(composite_confidence)
        print(f"Dynamic token confidence threshold: {self.token_confidence_threshold}")
        logger.info(f"Dynamic token confidence threshold updated to: {self.token_confidence_threshold}")
        ...
        normalized_actions = recovered_actions * amplification_factor
    ...
    def step(
        ...
        # language_instruction = "pick coke can and flip upright"
        # language_instruction = "push coke can with closed gripper"
        # language_instruction = "knock coke can over"
        # language_instruction = "stack the apple on the sponge"
        ...
        system_prompt = f"{DEFAULT_IMAGE_TOKEN}\nThe task is {language_instruction}. What is the action that the robot should take. To figure out the action that the robot should take to {language_instruction}, let's think through it step by step. First, what is the depth map for this image? Second, what is the trajectory of the end effector? Based on the depth map of the image and the trajectory of the end effector, what is the action that the robot should take?"
        # Process the image
        ...
        # print(f"Chosen token {chosen_token_ids}")
        ...
        # print(f"Chosen Prob {chosen_probs}")
        text_outputs = self.tokenizer.batch_decode(gen_out.sequences, skip_special_tokens=True)
        generated_text = text_outputs[0]
        print("Generated text:", generated_text)

        # match = re.search(r"The depth map for the image is *<DEPTH_START>(.*?)<DEPTH_END>", generated_text)
        # --- Extract depth tokens from the generated text ---
        # match = re.search(r"((?:<DEPTH_\d+>)+)<DEPTH_END>", generated_text)
        # if match:
        #     depth_tokens = match.group(1)
        # else:
        #     depth_tokens = ""
        #     logger.warning("No depth tokens found in the generated text.")
        ...
        # print(match.group(1))
        # depth_tokens = match.group(1)
        # print("Depth Tokens:", depth_tokens)
        ...
        if "The trajectory of the end effector is" in generated_text:
            try:
                # Split the generated text on the known phrase.
                traj_part = generated_text.split("The trajectory of the end effector is")[-1]
                # The trajectory is before the next period or the phrase "Based on"
                ...
                # if action_confidence < self.token_confidence_threshold:
                ...
                # if False:
                    ...
                    # Generate system_prompt
                    system_prompt = f"{DEFAULT_IMAGE_TOKEN}\nThe task is {language_instruction}. Notice that the trajectory of the end effector is annotated on the image. Based on the the trajectory annotated on the image, what is the action that the robot should take?"
                    # Process the image
                    image_tensor = process_images([img], self.image_processor, self.model.config)
                    image_tensor = [_image.to(dtype=torch.float16, device='cuda') for _image in image_tensor]
    ...
    def compute_confidence_for_token_list(self, token_list, chosen_probs):
        ...
        if confs:
            ...
            return 0.0
    # def compute_composite_confidence_for_token_list(self, token_list, chosen_probs):
    #     """
    #     Compute the composite confidence as the average probability over all tokens.
    #     """
    #     confs = []
    #     for token in token_list:
    #         for tok, prob in chosen_probs:
    ...
    def compute_composite_confidence_for_token_list(self, token_list, chosen_probs):
        """
        Compute the composite confidence as the average probability over all tokens 
        except those equal to the minimum confidence. If all tokens have the same 
        probability, return that value.
        """
        confs = []
        for token in token_list:
            for tok, prob in chosen_probs:
        ...
        if not confs:
            return 0.0
        min_conf = min(confs)
        # Filter out tokens that are equal to the minimum confidence.
        remaining = [p for p in confs if p != min_conf]
        if remaining:
            return sum(remaining) / len(remaining)
        else:
            # If all tokens have the same confidence (or only one token), return the min_conf.
            return min_conf


    def compute_min_confidence_for_token_list(self, token_list, chosen_probs):
        """
        For each token in token_list, look up its probability in chosen_probs
        and return the minimum probability as the confidence.
        """
        confs = []
        for token in token_list:
            for tok, prob in chosen_probs:
    ...
    def adjust_threshold(self, current_confidence: float):
        """
        Dynamically adjust the token confidence threshold based on the current inference confidence.
        New threshold is updated as:
            new_threshold = old_threshold + adjustment_factor * (current_confidence - old_threshold)
        """
        old_threshold = self.token_confidence_threshold
        self.token_confidence_threshold = old_threshold + self.threshold_adjustment_factor * (current_confidence - old_threshold)
        print(f"Threshold adjustin factor: {self.threshold_adjustment_factor}")
        logger.info(f"Adjusted threshold from {old_threshold} to {self.token_confidence_threshold} based on current confidence: {current_confidence}")
...
#     @staticmethod
#     def convert_action_string_to_floats(action_str: str) -> list:
#         """
#         Given an action string of the form:
#           <ACTION_START><ACTION_73><ACTION_184><ACTION_87><ACTION_132><ACTION_168><ACTION_255><ACTION_0><ACTION_END>
#         this function extracts the 7 inner tokens and converts each to a float in [-1, 1]
#         using the formula:
#           float_value = -1.0 + 2.0 * (int(token) / 255)
#         """
#         if not (action_str.startswith("<ACTION_START>") and action_str.endswith("<ACTION_END>")):
#             raise ValueError("Action string must start with <ACTION_START> and end with <ACTION_END>.")
#         inner_str = action_str[len("<ACTION_START>"):-len("<ACTION_END>")]
#         inner_tokens = re.findall(r"<ACTION_(\d+)>", inner_str)
#         if len(inner_tokens) != 7:
#             raise ValueError(f"Expected exactly 7 inner tokens, got {len(inner_tokens)}: {inner_tokens}")
#         float_values = []
#         for token in inner_tokens:
#             val = int(token)
#             float_val = -1.0 + 2.0 * (val / 255.0)
...
#         Input:
#             image: np.ndarray of shape (H, W, 3), uint8
#             task_description: Optional[str], task description; if different from previous, resets policy state
#         Output:
#             raw_action: dict; raw policy action output
...
#             try:
#                 traj_part = generated_text.split("<DEPTH_END>.")[1]
#                 traj_part = traj_part.split("Action:")[0]
#                 traj_str = traj_part.replace("Trajectory:", "").strip().rstrip('.')
#                 trajectory = ast.literal_eval(traj_str)
