        ...
        if "The trajectory of the end effector is" in generated_text:
            try:
                ...
                if False:
                    ...
                    # Generate system_prompt
                    system_prompt = f"{DEFAULT_IMAGE_TOKEN}\nThe task is {language_instruction}. The depth map for the image is <DEPTH_START>{depth_tokens}<DEPTH_END>. Notice that the trajectory of the end effector is annotated on the image. Based on the depth map and the trajectory annotated on the image, what is the action that the robot should take?"
                    # Process the image
                    image_tensor = process_images([img], self.image_processor, self.model.config)
                    image_tensor = [_image.to(dtype=torch.float16, device='cuda') for _image in image_tensor]
