...
####EPOCH 2.5
# declare -a arr=("/gscratch/cse/jason328/checkpoint/checkpoint-109000")
declare -a arr=("/gscratch/cse/jason328/checkpoint/unified-3")


#####EPOCH1
# declare -a arr=("/gscratch/cse/jason328/checkpoint/chk_1epoch")
...
# declare -a coke_can_options_arr=( "upright=True" "laid_vertically=True" "lr_switch=True")
# declare -a coke_can_options_arr=("lr_switch=True" "upright=True" )
...
do CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference.py --policy-model llava --ckpt-path ${ckpt_path} \
...
CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference.py --policy-model llava --ckpt-path ${ckpt_path} \
  --robot google_robot_static \
  --control-freq 3 --sim-freq 513 --max-episode-steps 80 \
  --env-name ${env_name} --scene-name ${scene_name} \
  --robot-init-x 0.35 0.35 1 --robot-init-y 0.20 0.20 1 --obj-init-x -0.35 -0.12 5 --obj-init-y -0.02 0.42 5 \
